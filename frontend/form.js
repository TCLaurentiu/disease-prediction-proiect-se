  ...
  function f(e = null) {
      for (var t of ((v.innerHTML = ""), l))
          if (t.selected) !w(t.value) && g(t);
              ...
              (n.innerHTML = t.label), (n.dataset.value = t.value), e && match(t.label.toLowerCase(), e.toLowerCase()) ? v.appendChild(n) : e || v.appendChild(n);
          }
  }

  function match(sympthom, search){
    sympthom_words = sympthom.split(" ");
    search_words = search.split(" ");

    let matches = true;
    for(let i=0;i<search_words.length;i++){
      let word_matches = false;
      if (sympthom_words[i] == ""){
        continue;
      }
      for(let j=0;j<sympthom_words.length;j++){
        if (sympthom_words[j].startsWith(search_words[i])){
          word_matches = true;
          break;
        }
      }
